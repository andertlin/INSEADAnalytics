...
**Your Answers here:**
<br>
1. 6 principal components<br>
2. 5yr T-Notes, 10yr T-Notes, Treasury Bonds, 2yr T-Notes, Candian Bond, Eurodollar, Euro-Bond, Euro-Bobl, Gilts UK, Euro-Schatz<br>
3. Yes, the combination of the US Treasuries, a basket of currencies, and few stock indices can explain the majority of the variances. Principle Component Factors<br>
4. 
```{r echo=FALSE, comment=NA, warning=FALSE, error=FALSE,message=FALSE,results='asis'}
...
show_data3 = data.frame(cbind(colnames(precrisis_data), round(cor(precrisis_data),2)))
m3<-gvisTable(show_data3,options=list(width=1920, height=min(400,27*(nrow(show_data3)+1)),allowHTML=TRUE))
print(m3,'chart')

Variance_Explained_Table_results3 <-PCA(precrisis_data, graph=FALSE)
Variance_Explained_Table3 <-cbind(paste("component",1:ncol(precrisis_data),sep=" "),Variance_Explained_Table_results3$eig)
...
m3<-gvisTable(show_data3,options=list(width=1920, height=min(400,27*(nrow(show_data3)+1)),allowHTML=TRUE,page='disable'),formats=list(Eigenvalue="#.##",Percentage_of_explained_variance="#.##",Cumulative_percentage_of_explained_variance="#.##"))
print(m3,'chart')
...
corused3 = cor(precrisis_data[,apply(precrisis_data!=0,2,sum) > 10, drop=F])
...
Rotated_Factors3 <- Rotated_Factors3[sorted_rows3,]
...
m3<-gvisTable(show_data3,options=list(showRowNumber=TRUE,width=1220, height=min(400,27*(nrow(show_data)+1)),allowHTML=TRUE,page='disable'))
print(m3,'chart')

```
<br>
5. We see more agricultural commodities in the components in the crisis period. We can use a smaller group of components to explain the when it's bearish.The correlations between assests are different between bull and bear markets.
...
**Your Answers here:**
<br>
1. This strategy allocates the daily investment based on the weighting of the +/- signs calcualted based on the moving average function ma. 
<br>
2. 
```{r echo=TRUE, eval=TRUE, comment=NA, warning=FALSE,error=FALSE, message=FALSE, prompt=FALSE, tidy=TRUE}
signal_used2 = 0*futures_data # just initialize the trading signal to be 0
# Take many moving Average (MA) Signals and let them "vote" with their sign (+-1, e.g. long or short vote, for each signal)
MAfreq2 <-seq(1,300,by=2)
...
show_data4 = data.frame(cbind(rownames(pnl_matrix(trading_strategy2)), round(pnl_matrix(trading_strategy2),2)))
m4<-gvisTable(show_data4,options=list(width=1220, height=min(400,27*(nrow(show_data4)+1)),allowHTML=TRUE))
print(m4,'chart')
```
<br>
<br>
