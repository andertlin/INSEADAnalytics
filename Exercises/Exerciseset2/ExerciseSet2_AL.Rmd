...
```
<br>
5. First, there are more agricultural commodities in the components in the crisis period. Second, we can use a smaller group of components to explain the market in the crisis period.The correlations between assests are different between bull and bear markets.
...
**Your Answers here:**
<br>
1. First it breaks down the time series data into windows. Then it calculates the moving averages of those windows. Transform the ma into 1, 0, and -1 and sum them together. If it is postive then you invest in that future. Scale the daily investments so they add up to $1 everyday. It only uses the information from two days before any given day.
<br>
2. 
```{r echo=TRUE, eval=TRUE, comment=NA, warning=FALSE,error=FALSE, message=FALSE, prompt=FALSE, tidy=TRUE}
